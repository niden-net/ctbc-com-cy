	...
	public function pe_theme_metabox_config_view() {
		...
		foreach ($this->post_types() as $name=>$pt) {
			...
			if (count($posts) > 0) {
				...
				if ( 'slide' !== $name && defined( 'ICL_LANGUAGE_CODE' ) ) {

					$content["wpml_supress_language_filter"] = 
						array(
							"label"=>__("WPML ignore language",'Pixelentity Theme/Plugin'),
							"type"=>"RadioUI",
							"description" => __("If set to yes, the view will ignroe the active WPML language.",'Pixelentity Theme/Plugin'),
							"options"=> array(
	...
	public function conf($id = null) {
		...
		if (is_object($id)) {
			...
			default:
				...
				if ( isset( $meta->{"post-$ptype"} ) ) {

					$conf->data = $meta->{"post-$ptype"};

				} else {

					$conf->data = new StdClass();
